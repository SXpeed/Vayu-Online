...
/**
 * The newsletter box needs nothing from the API, so it is rendered straight
 * away; the announcement bar and the hero come from the admin panel and are
 * drawn when data/store.js has the answer. Splitting the two is what lets
 * this run without /api/nav in front of the first paint.
 */
export function renderStaticSiteContent() {
    renderNewsletter();
}

/** Re-runnable: called again whenever data/store.js swaps in live copy. */
