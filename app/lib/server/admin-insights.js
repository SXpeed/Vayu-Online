...
import { json, ok, resource } from './http.js';
...
export async function analytics({ store }) {
  ...
  const [series, topPages, topProducts, recent, funnelRow, orders30d, searches, zeroResults, abandoned] =
    await Promise.all([
      trafficSeries(store, days),
      store.all(
      ...
      store.all('SELECT q, results, sid, t FROM searches ORDER BY t DESC LIMIT 40'),
      ...
      store.all(
        `SELECT lower(q) AS q, COUNT(*) AS count FROM searches WHERE results = 0
          GROUP BY lower(q) ORDER BY count DESC LIMIT 12`),
      ...
      // 'pending:%' rows are in-flight payments, not abandoned carts.
      store.all(
        `SELECT sid, items, updated_at AS t FROM carts
          WHERE updated_at < ? AND sid NOT LIKE 'pending:%'
          ORDER BY updated_at DESC LIMIT 30`, new Date(Date.now() - 3600000).toISOString()),
    ]);
  ...
  return json(200, {
    ...
    searches,
    zeroResults,
    abandoned: abandoned.map(c => ({ sid: c.sid, t: c.t, items: safeJson(c.items) })),
  });
}
...
export async function inventory({ store }) {
  const [log, alerts] = await Promise.all([
    ...
    store.all(
      `SELECT s.*, p.name AS product_name FROM stock_alerts s
         LEFT JOIN products p ON p.id = s.product_id ORDER BY s.t DESC LIMIT 100`),
  ]);
  return json(200, {
    log,
    alerts: alerts.map(a => ({
      productId: a.product_id, productName: a.product_name || '', email: a.email,
      notified: !!a.notified, t: a.t,
    })),
  });
}
...
export async function withItems(store, orders, { timeline = false } = {}) {
  ...
  return orders.map(o => ({
    ...
    shipping: o.shipping,
    total: o.total,
    payment: { method: o.payment_method, paymentId: o.payment_id, orderId: o.payment_order_id },
    guest: !!o.guest,
    status: o.status,
