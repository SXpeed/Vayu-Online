...
// the admin database on first run; the journal pages call hydrateJournal()
// below, which swaps the live set in when the API answers.
import { hydrateCatalogue, site } from '#lib/stores/site.svelte.js';
...
export let STORIES = STATIC_STORIES;

/**
 * Pull the admin panel's stories in. Awaited by the journal, article and
 * press pages before their first render; nothing else on the site pays for
 * it. Resolves to false when the API is unreachable, leaving the static
 * stories above in place.
 */
export async function hydrateJournal() {
    await hydrateCatalogue();
    const live = site.journal;
