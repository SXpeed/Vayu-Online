 ...
 *
 * A product spans five tables (itself, categories, gallery, variants,
 * tags), so every write here goes through `writeProduct`, which replaces
 * the child rows in one batch. Editing a product is therefore atomic:
 * either the new gallery and variants are all in, or none of them are.
 */
...
async function writeProduct(store, id, data, { created = false } = {}) {
  ...
  // Variant ids are minted here rather than in the loop, because nextId is
  // a query and a batch must be built before it is sent.
  const variantIds = [];
  ...
  data.variants.forEach((v, i) => statements.push(store.stmt(
    'INSERT INTO product_variants (id, product_id, label, price, stock, sort_order) VALUES (?, ?, ?, ?, ?, ?)',
    variantIds[i], id, v.label, v.price, v.stock, i)));
