...
import { now, normPhone, formatPrice } from './db.js';
import { productById, productByCatIdx, totalStock } from './catalogue.js';
import { currentCustomer } from './sessions.js';
import { checkoutDetails, loadAddresses, rememberAddress, CHECKOUT_FIELDS } from './accounts.js';
...
/**
 * Razorpay orders awaiting confirmation.
 *
 * The Node server kept these in a module-level Map, which cannot work
 * across isolates: the browser may confirm against a different instance
 * than it started on. They live in D1 instead, in the same `carts`-like
 * spirit — a short-lived row keyed by the gateway's order id. Entries
 * older than an hour are swept, so an abandoned payment window cannot pin
 * a cart forever.
 */
const PENDING_TTL_MS = 60 * 60 * 1000;

async function putPending(store, rzpOrderId, prep) {
  await store.batch([
    store.stmt('DELETE FROM carts WHERE sid LIKE ? AND updated_at < ?',
      'pending:%', new Date(Date.now() - PENDING_TTL_MS).toISOString()),
    store.stmt(
...
async function takePending(store, rzpOrderId) {
...
async function createRazorpayOrder(payment, prep) {
  ...
  if (!res.ok) return { error: rz.error?.description || 'Payment gateway error' };
  return { id: rz.id };
}

/* ---------- routes ---------- */

export async function checkout({ store, request, body }) {
  ...
  const account = await currentCustomer(store, request);
  ...
  const online = payment.provider === 'razorpay' && payment.razorpayKeyId && payment.razorpayKeySecret;

  if (online) {
    // The order is only recorded once /api/checkout/confirm verifies the
    // signature; until then it exists solely as a pending row.
    const rz = await createRazorpayOrder(payment, prep);
...
export async function validateCoupon({ store, request, body }) {
