...
import { hashPassword, verifyPassword, now } from '#shared/database/store.js';
import { json, ok, badRequest, notFound, unauthorized, methodNotAllowed } from '#shared/utils/http.js';
import {
...
async function register({ store, request, body }) {
  ...
  // Carry the guest wishlist into the new account, same as login.
  if (body.guestKey) await mergeGuestWishlist(store, customer.id, body.guestKey);
...
async function login({ store, request, body }) {
  ...
  // A guest may have built a wishlist under their browser's temp id before
  // signing in. Promote those rows onto the account now, de-duping against
  // what the account already saved.
  if (body.guestKey) await mergeGuestWishlist(store, customer.id, body.guestKey);
  const { token, maxAge } = await customerSession(store, customer.id);
...
export async function route(ctx, section) {
  ...
  // Wishlist is reachable signed-in or as a guest — the handler enforces
  // ownership (customer_id for a signed-in shopper, guest_key otherwise),
  // so it does not sit behind the `!customer` gate.
  if (section === 'wishlist') return handleWishlist(full);
