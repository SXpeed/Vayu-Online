...
export function getAuth(env) {
    ...
    const auth = betterAuth({
        ...
        advanced: {
            ...
            // Derived from PUBLIC_ORIGIN rather than hardcoded. This list
            // used to name one specific production domain, which meant the
            // shop could only ever be deployed to that hostname: on any
            // other origin Better Auth rejects the callback and sign-in
            // fails with no error the user can act on. The localhost entries
            // stay so `wrangler dev` works out of the box on either port.
            trustedOrigins: [
                env.PUBLIC_ORIGIN,
                env.PUBLIC_ORIGIN?.replace('://', '://www.'),
