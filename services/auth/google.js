...
import { json, redirect, parseCookies, badRequest } from '#shared/utils/http.js';
...
function readIdToken(idToken) {
  try {
    ...
    // Everything here is inside the try on purpose. atob() throws on a
    // malformed segment, and it used to sit outside — so a truncated or
    // non-base64 token raised past the caller and became a 500 instead of
    // the "signin=failed" redirect the callback already handles. Base64url
    // is also unpadded, so the padding is restored before decoding rather
    // than relying on atob's tolerance for it.
    const b64 = payload.replaceAll('-', '+').replaceAll('_', '/');
    const bytes = atob(b64.padEnd(Math.ceil(b64.length / 4) * 4, '='));
    // The claims are UTF-8 (a name may be in any script), and atob yields
    // one char per byte — TextDecoder is what turns those bytes back into
    // the right string.
...
export async function callback(ctx) {
  ...
  response.headers.append('Set-Cookie', customerCookie(token, maxAge, request));
  response.headers.append('Set-Cookie', clearState);
  return response;
}
