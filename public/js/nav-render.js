...
/**
 * Home page, Curated Categories. This was a hand-written rail of five
 * cards, so Accents and Souvenir were missing from it — the same drift
 * that had already hidden them from the phone menu.
 */
function renderCuratedGrid() {
    const grid = document.getElementById('curatedGrid');
    if (!grid) return;
    grid.innerHTML = entries().map(([slug, cat]) => `
