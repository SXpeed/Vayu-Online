...
function productRow(p, lowStockAt, selected) {
    ...
    const meta = [
        ...
        p.variants?.length ? `${p.variants.length} option(s)` : '',
...
export function productEditor(prod) {
    ...
    const modal = openModal(`
        ...
        <div class="form-grid">
            ...
            <div class="field full"><label>Variants <span style="text-transform:none;letter-spacing:0">(size / finish options — leave price empty to use the base price)</span></label>
                <div id="p-variants" style="display:grid;gap:8px"></div>
                <button type="button" class="btn small" id="p-var-add" style="margin-top:8px">+ Add variant</button>
                <div class="help">With variants, stock is tracked per variant and the base stock field is ignored.</div></div>
    ...
    /* ---- variants ---- */
    const varEl = $('#p-variants', modal);
    const drawVariants = () => {
        varEl.innerHTML = variants.map((v, i) => `
            <div style="display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:8px" data-i="${i}">
                <input placeholder="Label (e.g. Small)" value="${esc(v.label)}" data-f="label">
                <input placeholder="Price ₹ (optional)" type="number" min="0" value="${v.price ?? ''}" data-f="price">
                <input placeholder="Stock" type="number" min="0" value="${v.stock ?? 0}" data-f="stock">
                <button type="button" class="btn small danger" data-act="rm-var">✕</button>
            </div>`).join('');
    };
    drawVariants();

    varEl.addEventListener('input', (e) => {
        const row = e.target.closest('[data-i]');
        if (row) variants[Number(row.dataset.i)][e.target.dataset.f] = e.target.value;
    });
    varEl.addEventListener('click', (e) => {
        const btn = e.target.closest('button[data-act=rm-var]');
        if (btn) { variants.splice(Number(btn.closest('[data-i]').dataset.i), 1); drawVariants(); }
    });
    $('#p-var-add', modal).addEventListener('click', () => {
        variants.push({ label: '', price: '', stock: 0 });
        drawVariants();
    });
    ...
    $('#p-save', modal).addEventListener('click', async () => {
        ...
        const body = {
            ...
            variants: variants.filter(v => String(v.label).trim()),
