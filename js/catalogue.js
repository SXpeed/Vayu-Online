 ...
 * Single source of truth for the collection grid, the product detail page
 * and header search. `idx` in /pages/product.html?cat=&idx= is the position
 * of an item in its category array, so never reorder these lists.
 */
...
/** Flat list for search: every product with its category and index. */
export const allProducts = Object.entries(productData).flatMap(
