...
const EXACT = {
    ...
    'POST /api/account/wishlist/merge': account.wishlistMerge,
    'POST /api/account/wishlist/clear': account.wishlistClear,
};
...
const SECTION = {
    ...
    'POST /api/account/wishlist': account.wishlistAdd,

    'POST /api/admin/products': admin.product,
    'PUT /api/admin/products': admin.product,
    'POST /api/admin/categories': admin.category,
    'POST /api/admin/press': admin.pressEntry,
    // A PUT is a merge, not a replacement: every one of these handlers
    // writes `body.x ?? existing.x`, so sending only the field that changed
    // is the intended way to edit one. The full schema rejected that —
    // "expected string, received undefined" for a title nobody was
    // changing — so an edit had to resend the whole row or fail. The panel
    // does resend it and never noticed; anything else could not edit at all.
    //
    // Reordering the categories is the case that made this concrete: moving
    // one up the home page sends nothing but its new position, and had to
    // carry a title, a picture and every sub-category along with it to get
    // past a schema that only ever handed them straight back to the row
    // they came from.
    'PUT /api/admin/categories': admin.category.partial(),
    'PUT /api/admin/press': admin.pressEntry.partial(),
    'POST /api/admin/artists': admin.artistProfile,
    'PUT /api/admin/artists': admin.artistProfile.partial(),
    'POST /api/admin/events': admin.showEvent,
    'PUT /api/admin/events': admin.showEvent.partial(),
